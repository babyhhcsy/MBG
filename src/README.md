###MBG生成的EntityMapper介绍之GeneratedCriteria
####1、该类包含了一个抽象的类`GeneratedCriteria`该类主要是进行拼接查询语句的代码实例：
```java
...
> 注意下面的问题
 * 该类是个抽象的static静态类
 * protected修饰的该类，所以在其他java文件中是无法调用的；
 * 该类包含了每一个属性的查询条件，譬如大于小于，等于，是否包含等相关内容

###2、GeneratedCriteria构造器，在创建该类的时候就创建一个ArrayList
```java
 protected GeneratedCriteria() {
            super();
            criteria = new ArrayList<Criterion>();
...
###3、该类中属性介绍：
`criteria`是个list，其类型是: List<Criterion> 用户添加查询条件；循环进行拼接sql语句

####4、该类中使用到的方法介绍：
##### addCriterion用于拼接sql语句；将查询的条件进行拼接；
```java
 protected void addCriterion(String condition) {
            if (condition == null) {
                throw new RuntimeException("Value for condition cannot be null");
